...
func dynamic_heuristic_evaluation_function(grid [8][8]Square, my_color Square) float64 {
	...
	if my_front_tiles > opp_front_tiles {
		f = -float64(100*my_front_tiles) / float64(my_front_tiles+opp_front_tiles)
	} else if my_front_tiles < opp_front_tiles {
		f = float64(100*opp_front_tiles) / float64(my_front_tiles+opp_front_tiles)
	} else {
		f = 0.0
	}
	...
	if my_tiles > opp_tiles {
		m = float64(100*my_tiles) / float64(my_tiles+opp_tiles)
	} else if my_tiles < opp_tiles {
		m = -float64(100*opp_tiles) / float64(my_tiles+opp_tiles)
	} else {
		m = 0
	}
 ...
 * Assuming my_color stores your color
 */
func dynamic_heuristic_evaluation_function_alt(grid [8][8]Square, my_color Square) float64 {
	my_tiles := 0
	opp_tiles := 0
	my_front_tiles := 0
	opp_front_tiles := 0
